...
public class ComplexPreferences //extends br.com.kots.mob.complex.preferences.ComplexPreferences
{
    ...
    //protected ComplexPreferences complexPreferences;
    protected WeakReference<SharedPreferences> mWeakSharedPreferences;
    protected WeakReference<SharedPreferences.Editor> mWeakEditor;
    ...
    protected ComplexPreferences(Context context, String namePreferences, int mode) {
        ...
        mWeakSharedPreferences = new WeakReference<SharedPreferences>(context.getSharedPreferences(namePreferences, mode));
        mWeakEditor = new WeakReference<SharedPreferences.Editor>(context.getSharedPreferences(namePreferences, mode).edit());
    }
    ...
    public void putObject(Context context, String key, Object object) {
        ...
        if (object instanceof Bundle) {
            ...
            SharedPreferences.Editor editor = mWeakEditor.get();
            if (editor != null) {
    ...
    public <T> T getObject(String key, Class<T> a) {
        SharedPreferences preferences = mWeakSharedPreferences.get();
        if (preferences != null) {
...
//		}
        ...
        try {
            FileHelper.getInstance().writeStringFile(context, key, object.toString());
...
//		editor.putStringSet(key,objToPut);
    ...
    public static Object getJson(Context context, String key) {
        String jsonStr = FileHelper.getInstance().readStringFile(context, key);
        if (jsonStr == null) {
            return null;
        }
...
//		//throw new IllegalArgumentException("object is not a json or is null");
    ...
    public static void putSerializable(Context context, String key, Serializable object) {
        ...
        FileHelper.getInstance().storeSerializable(context, key, object);
...
//		putBundle(key, bu);
    ...
    //-----------------------------------------------------------------------------
    public static void putSerializable(Context context, String key, Serializable object, FileWriteCallback listener) {
        if (context == null || key == null || object == null) {
            return;
        }
        FileHelper.getInstance().storeSerializable(context, key, object, listener);
    }
    ...
    public static Object getSerializable(Context context, String key) {
        ...
        return FileHelper.getInstance().readSerializable(context, key);
...
//			//FileUtil.getInstance().writeStringFile(context, key, SerialBundle.serializeBundle(object));
        FileHelper.getInstance().storeBundle(context, key, object);
...
//		}
//		else{
        returnBundle = FileHelper.getInstance()
                .readBundle(context, key);//SerialBundle.deserializeBundle(FileUtil.extStorageDir(context)+key);
